...
class DOAPIClient:
    ...
    def ensure_default_firewall(self, name="doconsole-ssh-only", tags=None):
        """Find-or-create the shared SSH-only firewall. If `tags` is given, the firewall is
        created targeting those tags (DO applies it to any droplet carrying at least one of
        them, present or future - no per-droplet attach call needed); an existing firewall
        found by name is reused as-is either way."""
        ...
        firewall = self.create_firewall(name, inbound_rules, DEFAULT_OUTBOUND_RULES, tags=tags or None)
        return firewall["id"]
