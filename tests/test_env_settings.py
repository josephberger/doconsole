...
def test_cli_args_take_priority():
    ...
    token, ssh_key, playbooks_dir, auto_upload, default_tags = resolve_settings(args, env=env)
...
def test_defaults_when_nothing_set():
    args = make_args()
    token, ssh_key, playbooks_dir, auto_upload, default_tags = resolve_settings(args, env={})
...
def test_default_tag_picked_up_from_env():
    args = make_args()
    _, _, _, _, default_tags = resolve_settings(args, env={"DOCONSOLE_DEFAULT_TAG": "doconsole"})
    assert default_tags == ["doconsole"]


def test_default_tag_comma_separated_produces_multiple():
    args = make_args()
    _, _, _, _, default_tags = resolve_settings(args, env={"DOCONSOLE_DEFAULT_TAG": "doconsole, ssh-only ,web"})
    assert default_tags == ["doconsole", "ssh-only", "web"]


def test_default_tag_blank_or_whitespace_is_empty_list():
    args = make_args()
    for blank in ("", "   ", None):
        env = {"DOCONSOLE_DEFAULT_TAG": blank} if blank is not None else {}
        _, _, _, _, default_tags = resolve_settings(args, env=env)
        assert default_tags == []
